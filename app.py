...
def form():
    if request.method == 'POST':
        ...
        new_item = {
            ...
            "datetime": datetime_obj.strftime("%Y-%m-%d %H:%M"),  # Save combined datetime
            "status": status
        }
...
def edit(item_id):
    if request.method == 'GET':
        ...
        name = request.form.get("name")
        operationtheatre = request.form.get("operationtheatre")  # Update variable name
        doctorassigned = request.form.get("doctorassigned")  # Update variable name
